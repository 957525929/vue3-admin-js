<template>
  <a-row type="flex" class="mb-30">
    <a-col flex="auto">

      <a-form layout="inline" :model="form_search" ref="form_search_refs">
        <a-form-item ref="name" label="角色类型" name="role">
          <a-select style="width: 120px" @focus="focus" ref="select" @change="handleChange">
        ...
        <a-form-item label="关键字" name="keyword">
          <a-select style="width: 120px" @focus="focus" ref="select" @change="handleChange" v-model:value="form_search.key">
        ...
        <a-input-search placeholder="input search text" class="mr-10 w-200" v-model:value="form_search.keyword" />
    ...
    <a-col flex="100px">
      <a-button type="primary" block @click="data.visible=true">新增用户</a-button>
    </a-col>

  </a-row>

  <a-table :loading="data.loading_table" :dataSource="data.dataSource" :columns="data.columns" :row-selection="rowSelection" bordered :pagination="false">
  ...
  <a-row class="pt-30">
    <a-col :span="4">
      <a-button @click="deleteConfirm(null)" :disabled="data.delete_id.length===0">批量删除</a-button>
    </a-col>
    <a-col :span="20">
      <a-pagination class="float-right" show-quick-jumper v-model:current="data.current_page" :total="data.total" @change="handlePage" />
    </a-col>
  </a-row>
  <ModelUser v-model:show="data.visible" v-model:rowId="data.rowId" @loadData="getUserList" />
</template>
    ...
    const rowSelection = {
      onChange: (selectedRowKeys, selectedRows) => {
        ...
        if (selectedRows.length > 0) {
          // console.log(selectedRows.map(item => item.member_id))
